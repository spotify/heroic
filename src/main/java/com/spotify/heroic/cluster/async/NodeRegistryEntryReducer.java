...
public final class NodeRegistryEntryReducer implements Callback.Reducer<NodeRegistryEntry, Void> {
    ...
    @Override
    public Void resolved(Collection<NodeRegistryEntry> results, Collection<Exception> errors,
            Collection<CancelReason> cancelled) throws Exception {
        handleErrors(errors);
        handleCancelled(cancelled);

        log.info(String.format("Updated cluster registry with %d nodes (%d failed, %d cancelled)", results.size(),
                errors.size(), cancelled.size()));
    ...
    private void handleError(final Exception error) {
        if (error instanceof RpcNodeException) {
            ...
            log.error(String.format("Failed to refresh metadata for %s: %s (%s)", e.getUri(), e.getMessage(), cause));
        } else {
