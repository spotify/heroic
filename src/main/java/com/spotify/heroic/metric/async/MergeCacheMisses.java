...
final class MergeCacheMisses implements Callback.Reducer<MetricGroups, MetricGroups> {
    ...
    private List<Callback<CachePutResult>> updateCache(Map<Map<String, String>, List<DataPoint>> cacheUpdates)
        ...
        for (final Map.Entry<Map<String, String>, List<DataPoint>> update : cacheUpdates.entrySet()) {
            ...
            queries.add(cache.put(key.getFilter(), group, key.getAggregation(), datapoints));
        }
    ...
    /**
     * Use a map from <code>{tags -> {long -> DataPoint}}</code> to deduplicate overlapping datapoints.
     *
     * These overlaps are called 'cache duplicates', which mean that we've somehow managed to fetch duplicate entries
     * from one of.
     *
     * <ul>
     ...
     * </ul>
     *
     * While this contributes to unnecessary overhead, it's not the end of the world. These duplicates are reported as
     * cacheDuplicates in RowStatistics.
     */
    private MergeCacheMisses.JoinResult joinResults(Collection<MetricGroups> results) {
        ...
        final AddCached cachedResults = addCached(resultSet, cacheResult.getResult());
        ...
        return new JoinResult(resultSet, cacheUpdates, Statistics
                .builder(statistics)
                .cache(new Statistics.Cache(cachedResults.getHits(), cacheConflicts, cachedResults.getConflicts(),
