...
public final class TimeSeriesTransformer implements Callback.DeferredTransformer<List<GroupedSeries>, MetricGroups> {
    ...
    @Override
    public Callback<MetricGroups> transform(List<GroupedSeries> result) throws Exception {
        if (!cache.isConfigured() || aggregation == null)
            return ConcurrentCallback.newReduce(execute(result), MetricGroups.merger());

        return ConcurrentCallback.newReduce(executeCached(result), MetricGroups.merger());
    }

    private List<Callback<MetricGroups>> execute(final List<GroupedSeries> result) throws Exception {
    ...
    private List<Callback<MetricGroups>> executeCached(List<GroupedSeries> result) throws CacheOperationException {
    ...
    private Callback<MetricGroups> buildCachedLookup(final MetricBackend backend, final Map<String, String> group,
        ...
        final CacheGetTransformer transformer = new CacheGetTransformer(cache) {
            @Override
            public Callback<MetricGroups> cacheMiss(Map<String, String> group, DateRange range) throws Exception {
    ...
    private Callback<MetricGroups> buildLookup(final MetricBackend backend, final Map<String, String> group,
        ...
        return ConcurrentCallback.newReduce(callbacks, buildReducer(group, range)).transform(MetricGroups.identity(),
    ...
    private Callback.StreamReducer<FetchData, MetricGroups> buildReducer(Map<String, String> group,
