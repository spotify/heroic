...
public class WebProfile extends HeroicProfileBase {
    @Override
    public HeroicConfig.Builder build(final ExtraParameters params) throws Exception {
        ...
        final boolean enableCors = params.getBoolean("enableCors").orElse(false);

        return builder.enableCors(enableCors);
    }
    ...
    public List<ParameterSpecification> options() {
        ...
        return ImmutableList.of(
            parameter("enableCors", "If set, use to determine if cors is enabled or not",
                "<boolean>")
        );
        // @formatter:on
    }
