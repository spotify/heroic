...
public class CoreShellTasks implements ShellTasks {
    ...
    @Override
    public ShellTask resolve(final String name) {
        return resolveTask(name);
    }

    private AsyncFuture<Void> runWithRedirectedIO(
        ShellIO io, final ShellTask task, final TaskParameters params
    ) throws IOException {
        final PrintWriter out =
            new PrintWriter(new OutputStreamWriter(setupOutputStream(io, params), Charsets.UTF_8));
    ...
    private AsyncFuture<Void> runTaskWithIO(
        final ShellTask task, final ShellIO io, final TaskParameters params
    ) {
        try {
    ...
    ShellTask resolveTask(final String taskName) {
        ...
        if (selected.size() > 1) {
            throw new IllegalArgumentException(
                String.format("Too many (%d) matching tasks (%s)", selected.size(),
