...
public class UtilsResource {
     ...
     * 
     * @Produces(MediaType.APPLICATION_JSON) public Response decodeRowKey(final String data) { final MetricsRowKey key =
     * decode(data, MetricsRowKeySerializer.get()); return Response.status(Response.Status.OK).entity(key).build(); }
     * 
     * @POST
     * 
     * @Path("/encode-row-key")
     ...
     * 
     * @Produces(MediaType.TEXT_PLAIN) public Response encodeRowKey(final MetricsRowKey key) { final String data =
     * encode(key, MetricsRowKeySerializer.get()); return Response.status(Response.Status.OK).entity(data).build(); } */
