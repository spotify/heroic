...
public abstract class DistributedBucketAggregation<B extends Bucket> extends BucketAggregation<B> {
    public DistributedBucketAggregation(final long size, final long extent, final Set<MetricType> input, final MetricType out) {
    ...
    public Aggregation distributed() {
        return new SpreadAggregation(getSize(), getExtent());
    }
