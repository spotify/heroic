...
package com.spotify.heroic.aggregation.simple.util;
...
public class SampleQuantile {
    ...
    /**
     * Buffers incoming items to be inserted in batch. Items are inserted into the buffer linearly. When the buffer
     * fills, it is flushed into the samples array in its entirety.
     */
    ...
    public SampleQuantile(double quantile, double error) {
    ...
    private double allowableError(int rank) {
        ...
        if (rank <= quantile * size) {
            error = (2.0 * this.error * (size - rank)) / (1.0 - quantile);
        } else {
            error = (2.0 * this.error * rank) / quantile;
        }
     ...
     * 
     * @return snapshot of the tracked quantiles. If no items are added to the estimator, returns null.
     */
    synchronized public Double snapshot() {
