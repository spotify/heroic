...
public class MaxAggregationQuery implements AggregationQuery {
    ...
    @JsonCreator
    public MaxAggregationQuery(@JsonProperty("sampling") SimpleSamplingQuery sampling) {
        this.sampling = Optional.fromNullable(sampling).transform(SimpleSamplingQuery::build);
    }

    @Override
    public MaxAggregation build(AggregationContext context) {
        return new MaxAggregation(sampling.or(context.getSampling()).or(SimpleSamplingQuery.DEFAULT_SUPPLIER));
    }
}