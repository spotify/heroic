...
/**
 * A rate per second implementation inspired by the prometheus irate() function.
 *
 * It is intended to be be used with counters that are increasing to calculate the rate of change
 * per second difference between two points. It handles the counter being reset.
 *
 * @author dmichel
 */
@Data
public class RatePerSecondInstance implements AggregationInstance {
    ...
    List<Point> computeDiff(List<Point> points) {
        ...
        while (it.hasNext()) {
            ...
            if (current.getValue() < previous.getValue()) {
                result.add(new Point(
                  current.getTimestamp(), current.getValue() * 1000.0 / timeDiff));
            } else {
