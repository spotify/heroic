...
## number of metadata consumers. if using autoscaling, set this to the autoscaler max
metadata_consumers <- 35
################################################################################
...
tree <- addLogic(tree, at=1, type="or", tag="BigTableCluster", name="Metrics BT Consumer", name2="cluster failure")
## PubSub failure
## https://cloud.google.com/pubsub/sla
tree <- addProbability(tree, at="MetadataCluster", tag="PubSub", name="PubSub failure", prob=0.0005)

## Host failure events
capacity_max <- ceiling(metadata_consumers - (metrics_per_second / consumer_capacity))
tree <- addLogic(tree, at="MetadataCluster", type="vote", vote_par=c(capacity_max, metadata_consumers),
...
## BT Consumers
tree <- addProbability(tree, at="BigTableCluster", tag="ZonalBT", name="Zonal failure", prob=0.001)
tree <- addLogic(tree, at="BigTableCluster", type="vote", vote_par=c(capacity_max, metadata_consumers),
                 tag="BTCapacity", name="Consumer", name2="capacity exceeded")
tree <- addLogic(tree, at="BTCapacity", type="or", tag="BTCapacityHosts",
                 name="BTCapacity", name2="Host failure")
tree <- addDuplicate(tree, at="BTCapacityHosts", dup_id="Host", collapse=TRUE)
