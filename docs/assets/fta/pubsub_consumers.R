...
## Host failure events
metadata_capacity_max <- ceiling(metadata_consumers - (metrics_per_second / consumer_capacity))
tree <- addLogic(tree, at="MetadataCluster", type="vote", vote_par=c(metadata_capacity_max, metadata_consumers),
                 tag="MetadataCapacity", name="Metadata consumer", name2="capacity exceeded")
tree <- addLogic(tree, at="MetadataCapacity", type="or", tag="Host", name="Host failure")
